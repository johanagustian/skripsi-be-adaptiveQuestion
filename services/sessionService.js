...
const getAllSessions = async (user_id) => {
    ...
    const query = `
        SELECT 
            s.session_id, 
            s.created_at,
            d.file_name, -- PASTIKAN: Sesuaikan dengan nama kolom nama file di tabel dokumenmu
...
const startSession = async (user_id, document_id) => {
    ...
    const abilityCheck = await pool.query('SELECT theta_score FROM user_abilities WHERE user_id = $1', [user_id]);
...
const getNextQuestion = async (session_id, user_id, document_id) => {
    ...
    const abilityQuery = await pool.query('SELECT theta_score FROM user_abilities WHERE user_id = $1', [user_id]);
    ...
    if (chunkResult.rows.length === 0) {
        throw new Error("Dokumen belum dipotong (chunking) atau tidak memiliki teks.");
    }
    const selectedChunk = chunkResult.rows[0].toLowerCase();
    ...
    try {
        ...
        const aiResponse = await axios.post(pythonApiUrl, {
            ...
            context: selectedChunk.context_text
        });
        ...
        return {
            ...
            reading_context: selectedChunk.context_text, // Frontend akan menampilkan teks ini di samping soal
        ...
        throw new Error('Gagal membangkitkan soal dari AI Service: ' + (error.response?.data?.detail || error.message));
    }
};
...
const submitAnswer = async (session_id, user_id, question_id, user_answer) => {
    ...
    const is_correct = (selectedText.trim().toLowerCase() === question.correct_answer.trim().toLowerCase());
    const S = is_correct ? 1 : 0; 
    const b_parameter = question.b_parameter;

    // C. Ambil Theta (Kemampuan) user saat ini
    const abilityResult = await pool.query('SELECT theta_score FROM user_abilities WHERE user_id = $1', [user_id]);
    ...
    await pool.query(
        ...
        [history_id, session_id, user_id, question_id, user_answer, is_correct, current_theta, new_theta]
    );

    // G. Timpa nilai Theta lama di tabel user_abilities
    await pool.query('UPDATE user_abilities SET theta_score = $1 WHERE user_id = $2', [new_theta, user_id]);
    ...
    return {
        ...
        new_theta: new_theta,
        explanation: is_correct ? "Tepat sekali!" : "Jawaban kurang tepat. Coba perhatikan lagi konteks bacaannya."
    };
};
...
const getSessionReview = async (session_id, user_id) => {
    const sessionCheck = await pool.query(`SELECT session_id FROM sessions WHERE session_id = $1`, [session_id]);
